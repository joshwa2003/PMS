...
const placementStaffProfileSchema = new mongoose.Schema({
  // Reference to User
  userId: {
  ...
  // Placement Staff Specific Fields
  assignedStudents: [{
    type: mongoose.Schema.Types.ObjectId,
    ref: 'Student'
  }],
  responsibilitiesText: {
    type: String,
    trim: true,
    maxlength: [1000, 'Responsibilities text cannot exceed 1000 characters']
  },
  trainingProgramsHandled: [{
    type: String,
    trim: true,
    maxlength: [200, 'Training program name cannot exceed 200 characters']
  }],
  languagesSpoken: [{
    type: String,
    trim: true,
...
placementStaffProfileSchema.virtual('fullName').get(function() {
  return `${this.name.firstName} ${this.name.lastName}`;
});

// Virtual for complete address
placementStaffProfileSchema.virtual('fullAddress').get(function() {
...
// Virtual for assigned students count
placementStaffProfileSchema.virtual('assignedStudentsCount').get(function() {
  return this.assignedStudents ? this.assignedStudents.length : 0;
});

// Index for better query performance
placementStaffProfileSchema.index({ userId: 1 });
placementStaffProfileSchema.index({ employeeId: 1 });
placementStaffProfileSchema.index({ email: 1 });
placementStaffProfileSchema.index({ department: 1 });
placementStaffProfileSchema.index({ role: 1 });
placementStaffProfileSchema.index({ status: 1 });

// Pre-save middleware to calculate profile completion
placementStaffProfileSchema.pre('save', function(next) {
...
placementStaffProfileSchema.statics.findByUserId = function(userId) {
...
placementStaffProfileSchema.statics.findByEmployeeId = function(employeeId) {
...
placementStaffProfileSchema.methods.updateProfileCompletion = function() {
  return this.save();
};

module.exports = mongoose.model('PlacementStaffProfile', placementStaffProfileSchema);
