...
// Reducer function
const administratorProfileReducer = (state, action) => {
  switch (action.type) {
...
// Create context
const AdministratorProfileContext = createContext();

// Administrator Profile Provider component
export const AdministratorProfileProvider = ({ children }) => {
  const [state, dispatch] = useReducer(administratorProfileReducer, initialState);
  const { user, isAdmin, updateProfilePicture } = useAuth();
  ...
  const getAccessLevelFromRole = (role) => {
    const roleAccessMapping = {
      'admin': 'admin',
      'placement_director': 'admin',
      'placement_staff': 'limited',
      'department_hod': 'limited',
      'other_staff': 'limited'
    };
    return roleAccessMapping[role] || 'limited';
  };
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      // Try to get existing administrator profile first
      const profile = await administratorProfileService.getProfile();
  ...
  const saveProfile = async (sectionData = null) => {
    ...
    try {
      ...
      // Use administratorProfileService to update profile
      const updatedProfile = await administratorProfileService.updateProfile(cleanedData);
      
      dispatch({ type: ACTIONS.SET_PROFILE, payload: updatedProfile });
      
      return { success: true, profile: updatedProfile };
    } catch (error) {
      console.error('Save administrator profile error:', error);
      dispatch({ type: ACTIONS.SET_ERROR, payload: error.message });
  ...
  const getProfileCompletion = () => {
    return administratorProfileService.calculateProfileCompletion(state.formData);
  };
  ...
  return (
    <AdministratorProfileContext.Provider value={value}>
      {children}
    </AdministratorProfileContext.Provider>
  );
};

// Custom hook to use administrator profile context
export const useAdministratorProfile = () => {
  const context = useContext(AdministratorProfileContext);
  if (!context) {
    throw new Error('useAdministratorProfile must be used within an AdministratorProfileProvider');
  }
  return context;
};

export default AdministratorProfileContext;
